================================================================================
    MFC 라이브러리 : Act 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 Act 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

Act 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
Act.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CActApp 응용 프로그램 클래스가 선언되어 있습니다.

Act.cpp
    이는 응용 프로그램 클래스 CActApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\Act.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 Act.rc에 의해 포함됩니다.
...
ActDoc.h, ActDoc.cpp - 문서
    이 파일에는 CActDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CActDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      act
        파일 형식 ID:        Act.Document
        주 프레임 캡션:  Act
        Doc 형식 이름:       Act
        필터 이름:         Act Files (*.act)
        파일의 새 약식 이름: Act
        파일 형식의 긴 이름: Act.Document

ActView.h, ActView.cpp - 문서 뷰
    이 파일에는 CActView 클래스가 포함됩니다.
    CActView 개체는 CActDoc 개체를 보는 데 사용됩니다.
...
CntrItem.h, CntrItem.cpp
    이 파일에는 CActCntrItem 클래스가 포함됩니다. 이 클래스는 OLE 개체를 조작하는 데 사용됩니다. OLE 개체는 보통 CActView 클래스에 의해 표시되며 일부 CActDoc 클래스로 serialize됩니다.

SrvrItem.h, SrvrItem.cpp
    이 파일에는 CActSrvrItem가 포함됩니다. 이 클래스는 CActDoc 클래스를 OLE 시스템에 연결하는 데 사용되고, 문서에 대한 링크를 선택적으로 제공합니다.
    프로젝트에서 활성 문서를 만들고 관리할 수 있습니다.

IpFrame.h, IpFrame.cpp
    이 파일에는 CInPlaceFrame가 포함됩니다. 이 클래스는 COlelPFrameWnd에서 파생되었으며 내부 활성화 동안 모든 프레임 기능을 제어합니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 Act.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.
