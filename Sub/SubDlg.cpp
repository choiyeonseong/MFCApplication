...
CSubDlg::CSubDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_SUB_DIALOG, pParent)
{
...
BEGIN_MESSAGE_MAP(CSubDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_MESSAGE(WM_MY_MSG,OnMyUserProc)	// 메시지 맵 엔트리
	ON_WM_COPYDATA()
END_MESSAGE_MAP()
...
{
	AfxMessageBox(_T("메시지 받음"));
	CString str;
	LONG nID = (LONG)wParam;	// 전달받은 상대방 프로그램의 프로세스ID
	str.Format(_T("Process ID = %X"), nID);
...
BOOL CSubDlg::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)	// (WM_COPYDATA 메시지를 보낸 윈도우의 객체 포인터, WM_COPYDATA 메시지와 함께 전달된 COPYDATASTRUCT 구조체의 포인터)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	AfxMessageBox(_T("메시지 받음"));
	switch (pCopyDataStruct->dwData)	// 사용 용도를 판단
	{
	case 1004:	// COPYDATATSTRUCT 구조체의 dwData에 설정했던 값
		SetDlgItemText(IDC_DATA, (LPCTSTR)pCopyDataStruct->lpData);
		break;
	}
