...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// 플로팅 툴바
	m_wndToolBar.SetWindowText(_T("나만의 플로팅 툴바"));	// 플로팅 툴바의 캡션 설정
	CPoint pt;
	pt.x = GetSystemMetrics(SM_CXSCREEN) / 2;
	pt.y = GetSystemMetrics(SM_CYSCREEN) / 3;
	//FloatControlBar(&m_wndToolBar, pt);	// TODO : 형변환 오류 생김
	...
	CCreateContext* pContext)
{
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CLeftView), CSize(100, 100), pContext) ||
		!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CImageLView), CSize(100, 100), pContext))
	{
